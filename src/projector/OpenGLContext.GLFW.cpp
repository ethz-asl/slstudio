...
OpenGLContext::OpenGLContext(uint _screenNum)
  ...
  contextInfo->window =
      glfwCreateWindow(screenResX, screenResY, "GLFW OpenGL Context",
                       contextInfo->monitor, NULL);
  ...
  //    const char *glx_extensions = glXQueryExtensionsString(display,
  //    screenNum); if (strstr(glx_extensions, "GLX_SGI_swap_control")) {
  //        PFNGLXSWAPINTERVALSGIPROC SwapIntervalSGI =
  //        (PFNGLXSWAPINTERVALSGIPROC)glXGetProcAddressARB((const
  //        GLubyte*)"glXSwapIntervalSGI"); SwapIntervalSGI(1);
...
//    } else {
//        std::cerr << "OpenGLContext.Unix Error: Could not access swap interval
//        extension!" << std::endl;
//    }
#endif
  ...
  // Adjust gamma to one
  // This line was causing some problems so just commented it out
  // this->setGamma(1.0);
}
